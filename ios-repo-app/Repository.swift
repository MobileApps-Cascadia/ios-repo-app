...
class Repository<genericObject> where genericObject : Codable, genericObject: hasID  {
    var path: String
    ...
    func fetch(withId id: Int, withCompletion completion: @escaping (genericObject?) -> Void) {
        ...
        if let url = URL.init(string: URLstring){
            let task = URLSession.shared.dataTask(with: url, completionHandler: {(data, response, error) in
                if let music = try? JSONDecoder().decode(genericObject.self, from: data!){completion(music)}
            })
            task.resume()
        }
    }
